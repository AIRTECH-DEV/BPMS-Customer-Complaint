...
/*function onFormSubmit(e) {
  ...
  // We grab emails from all possible columns
  let emailList = [
  ...
  // Filter out empty strings and keep only unique, valid email addresses
  ...
  // --- 2. URL MAPPING (Updated for your new link) ---
  const prefilledUrl = baseUrl + 
    "&entry.1063906463=" + encodeURIComponent(getVal('Service Type')) +
    "&entry.1863661142=" + encodeURIComponent(getVal('Customer Type')) +
    ...
    // Residential Fields (Note: New IDs from your link added here)
    "&entry.1309771941=" + encodeURIComponent(getVal('Client Name')) +
    "&entry.1370847551=" + encodeURIComponent(getVal('Flat No')) + // Updated ID from your link
    "&entry.897794032="  + encodeURIComponent(getVal('Building')) + // Updated ID from your link
 ...
 // --- 3. SEND EMAIL FROM CUSTOMER CARE ---
  if (uniqueEmails.length > 0) {
    const siteName = getVal('Company Name') || getVal('Client Name') || "Valued Customer";
    const subject = "Service Link for: " + siteName;
    
    // Create the HTML Body with the hyperlink
    const htmlBody = `
      <p>Dear Customer,</p>
      <p>Please use the link below for future service requests. Your details have been pre-filled for your convenience:</p>
      <p><strong><a href="${prefilledUrl}">Click Here </a></strong></p>
      <p>Regards,<br>Vakharia Airtech Customer Care</p>
    `;

    // Plain text fallback for email clients that don't support HTML
    const plainTextBody = "Dear Customer,\n\nPlease use this link for future service requests: " + prefilledUrl;
    ...
    uniqueEmails.forEach(email => {
      try {
        GmailApp.sendEmail(email, subject, plainTextBody, {
          ...
          htmlBody: htmlBody // This renders the "Click Here" link
        });
        console.log("Sent to: " + email);
      } catch (err) {
        console.error("Failed to send to " + email + ". Check if the alias is authorized: " + err);
      }
    });
  }

  // --- 4. SAVE TO SHEET ---
  const targetColumn = 32; 
  sheet.getRange(row, targetColumn).setValue(prefilledUrl);
  if (sheet.getRange(1, targetColumn).getValue() === "") {
    sheet.getRange(1, targetColumn).setValue("Magic Link (WhatsApp)");
  }
}*/
